...
## Implementation Status (2026-08-10, all phases + Reports + Audit Log + Form 16 + Regime Lock complete)
...
- **Phase 7 – Form 16 & Regime Lock: complete.** `Form16Service::generate()` produces a per-employee, per-financial-year Form 16 PDF (`form16s` table, unique on `employee_id`+`financial_year_id`) reusing the same `IncomeTaxCalculationService::project()` figures payroll already trusts — no numbers are recomputed independently, so a Form 16 always matches what payroll actually deducted. It renders Part B (gross salary, standard deduction, verified Chapter VI-A/section-wise deductions for the old regime, taxable income, rebate/surcharge/cess, total tax) plus a quarter-wise TDS summary built from `payroll_run_employee_lines` (Apr-Jun/Jul-Sep/Oct-Dec/Jan-Mar), styled like Part A but explicitly disclaimed as not a TRACES-issued certificate (that requires the employer's quarterly 24Q return to be filed first — out of scope here). `companies.tan` (new column) prints alongside the existing `pan` on the certificate header. Generation is HR/Payroll-only (`tax.manage`) via a new **Generate Form 16** admin page (`/admin/generate-form16`, per-employee and bulk actions, financial-year picker); employees download their own via a new **My Form 16** ESS page, through a permission-gated `Form16DownloadController` route (`form16.download`) — same pattern as `payslips.download`. Separately, **admin can now actually fix/lock an employee's tax regime**: `TaxRegimeSelectionService` routes every `employee_tax_regimes` write (both self-service and admin-assisted, via `CreateEmployeeTaxRegime::handleRecordCreation()`) through a lock check that previously did nothing — `EmployeeTaxRegime.lock_date` (set by a new admin-only "Fix / Lock" table action on `EmployeeTaxRegimeResource`) or `TaxRegimeConfig.regime_change_allowed = false` (a blanket freeze for everyone under that FY/regime) each independently block further self-changes; `tax.manage` users always bypass both, which is how they "fix" an employee's regime regardless of lock state. An "Unlock" action reverses a row-level lock. 8 automated tests (`tests/Feature/Phase7SmokeTest.php`): self-select-when-unlocked, lock-blocks-self-change, admin-overrides-a-lock, unlock-restores-self-change, config-level freeze blocks independently of any row lock, Form 16 generation produces a downloadable PDF with the right regime/generated_by, and download permission gating (owner allowed, unrelated employee 403).
...
- **Work From Home module: complete.** A dedicated `WorkFromHomeRequest` (implements `Approvable`, single-level Reporting-Manager-only workflow — `WorkflowDefinition::MODULE_WORK_FROM_HOME`, seeded in `Phase2Seeder`) lets an employee request a date range; on approval, `WorkFromHomeService::applyApprovedDays()` marks every working day in the range `status = wfh` on the attendance register (skipping weekly-off/holiday/frozen/already-on-leave days). Employees then self-punch in/out for the day via a new **WFH Clock In/Out** ESS page (`App\Filament\Pages\WfhAttendance`, `source = self` on the punch — only accepted when today's attendance is already marked `wfh`), reusing the exact same first-in/last-out/late-minutes/effective-hours computation as biometric punches (`AttendanceService::recalculate()`). That method was widened with a guard so it only auto-assigns `present`/`half_day` when the status is already one of the generic present-family states — it no longer clobbers `wfh`/`on_duty`/`leave`/`holiday`/`weekly_off` just because a punch triggered a recalculation, while still computing late marks and incomplete-hours for those days. Two report surfaces, both scoped via the existing `ScopesToOwnTeam` (HR sees all, a Manager their team, an Employee only themselves): an on-screen **WFH Report** page (`App\Filament\Pages\WfhReport`, in/out time, hours completed, late-mark badge, Completed/Incomplete badge) and a "Work From Home" card on the existing Reports page downloading the same data as `.xlsx` (`WfhReportExport`, routed through the existing generic `ReportDownloadController`). No new permissions were added — visibility reuses `attendance.view`/`attendance.manage` exactly like Attendance Regularization.
- **Attendance Register (employee × day-of-month matrix): complete.** New on-screen page (`App\Filament\Pages\AttendanceRegister`) plus a matching `.xlsx` export (`AttendanceRegisterExport`, card on the Reports page) covering **all** attendance (not WFH-only, unlike the WFH Report above) — employees down the Y axis, one column per calendar day across the X axis, each cell showing first punch, last punch, and work hours (falling back to a short status code — WO/H/L/A — on days with no punches). Reuses `AttendanceMonthlySummaryService::buildForEmployee()`, the same per-day builder the existing compact-code Monthly Attendance View already used — that method was widened to also carry the raw `first_in`/`last_out`/`hours` per day (previously it only returned a `code`/`label` pair), a purely additive change that doesn't affect the two existing consumers of that method. The bg/text color palette used by both the Monthly Attendance View and its Excel export was also consolidated into one shared `AttendanceMonthlySummaryService::CODE_COLORS` constant (was duplicated 2x with identical values before) so the Attendance Register's status-code cells match the same palette everywhere. The whole "Attendance" nav group was also re-sequenced with unique `$navigationSort` values (two pairs had accidentally collided — Attendance Regularization/Biometric Devices both at 4, Work From Home/Device Punch Logs both at 5) into: Shifts, Holiday Calendar, Daily Attendance, Attendance Register, Monthly Attendance View, Regularization Requests, Work From Home, WFH Clock In/Out, WFH Report, Biometric Devices, Device Punch Logs. 3 automated tests (`tests/Feature/AttendanceRegisterSmokeTest.php`).
- **Form 16 for resigned employees now includes their Full & Final settlement earnings: fixed 2026-08-21.** `IncomeTaxCalculationService::projectAnnualIncome()`/`tdsDeductedTillDate()` previously summed *only* `payroll_run_employees` — for an employee who resigned, their last partial month's pay is entered as `FullFinalSettlement.pending_salary` (plus `bonus_incentive`/`other_earnings`) rather than run through a normal `PayrollRun`, so it was silently excluded from Form 16/tax projections entirely ("based on credit" — only what was formally run through payroll — instead of "based on salary" — what they actually earned). Fixed by adding `fnfTaxableEarnings()`/`fnfTdsDeducted()`, which sum `pending_salary + bonus_incentive + other_earnings` (and `tds`) from **PAID** `FullFinalSettlement`s whose resignation's `approved_last_working_date` falls inside the financial year. Deliberately excludes `leave_encashment` (Section 10(10AA) partial exemption rules aren't modeled anywhere in this codebase — including it here without that logic would overstate tax) and `reimbursement` (an expense repayment, not income) — a known simplification, flagged rather than guessed at. 2 tests (`tests/Feature/FnfIncomeTaxSmokeTest.php`).
- **Attendance display: same in/out time no longer shown twice — fixed 2026-08-21.** A single punch (e.g. only clocking in, or `BiometricPunchService`/`WorkFromHomeService` seeding both `first_in`/`last_out` from one punch when the other side is still null) used to render as identical in/out times everywhere. New `Attendance::hasDistinctPunches()`/`getDisplayLastOutAttribute()` treat that case as a missing out-punch; wired into `AttendanceMonthlySummaryService::cellForAttendance()` (covers Monthly Attendance View + Attendance Register + their exports), `AttendanceResource`'s table, `AttendanceReportExport`, and both WFH pages.
- **"8-hour complete/incomplete" highlighting on the Attendance Register: added 2026-08-21.** New `AttendanceService::minFullDayHoursFor()` (an employee's shift `min_full_day_hours`, falling back to a standard `DEFAULT_FULL_DAY_HOURS = 8.0` when no shift is assigned) drives green/red coloring on the hours figure in both the on-screen Attendance Register (badge on the hours line, reusing `AttendanceMonthlySummaryService::CODE_COLORS`' Present/Absent palette) and `AttendanceRegisterExport` (whole-cell fill, tracked via a per-employee/day completion matrix since Excel can't color a single line within a wrapped-text cell). The same helper now also backs `WfhReport`/`WfhAttendance`'s existing Completed/Incomplete badges, replacing their old "no shift = always incomplete" fallback. Both `AttendanceRegister` and `WfhReport` also gained a header "Export to Excel" action linking straight to the matching report-download route.
- **Microsoft Graph mail transport: added 2026-08-21, not yet activated.** New `App\Mail\Transport\MicrosoftGraphTransport` (registered via `Mail::extend('graph', ...)` in `AppServiceProvider`) sends every outgoing email through Microsoft Graph's `sendMail` API (app-only OAuth2 client-credentials against Entra ID, access token cached ~55 min) instead of SMTP — a `config/mail.php` `graph` mailer entry reads `MS_GRAPH_TENANT_ID`/`MS_GRAPH_CLIENT_ID`/`MS_GRAPH_CLIENT_SECRET`/`MS_GRAPH_SENDER` from `.env`. Purely additive: `MAIL_MAILER` is still `smtp` in both `.env` and `.env.example` — switching it to `graph` requires an Entra ID app registration with the Graph **application** permission `Mail.Send` (admin-consented) and filling in those four values first, which needs Azure portal access this session doesn't have. Every notification already goes out from one address today regardless of transport (`mail.from`, currently `MAIL_FROM_ADDRESS`) — no Notification class overrides it — so this only changes *how* mail is sent, not the fact that it's already sent from a single sender. 3 tests (`tests/Feature/MicrosoftGraphMailTransportTest.php`), mocking both the token and sendMail endpoints via `Http::fake()`.
...
| 15 | Income Tax | FY tax slabs (old/new regime), regime selection + admin lock, declarations, TDS, Form 16 |
...
| 22 | Work From Home | Manager-approved WFH date-range requests, self clock-in/out, WFH attendance report |
...
Attendance
  ...
  Daily Attendance / Bulk Import
  Attendance Register / Monthly Attendance View
  Regularization Requests
  Work From Home (request + manager approval)
  WFH Clock In/Out (ESS) / WFH Report
  Biometric Devices / Device Punch Logs
...
Income Tax
  Financial Years / Tax Slabs (Old/New) / Employee Regime (admin can Fix/Lock or Unlock)
  Investment Declarations / Proof Verification / Tax Comparison / Monthly TDS
  Generate Form 16 [HR, Payroll] / My Tax Comparison (ESS) / My Form 16 (ESS)
...
Employee (ESS) login sees only: Dashboard, My Profile, My Attendance, My Leave, My Expenses, My Salary, My Tax, My Form 16, My Loans, My Requests.
...
- **work_from_home_requests**: id, employee_id -> employees, from_date, to_date, reason, status (pending/approved/rejected/sent_back), approval_instance_id -> approval_instances. `total_days` (working-day count, excludes weekly-off/holiday) is computed on demand, not stored. On final approval, `WorkFromHomeService::applyApprovedDays()` marks every working day in range as an `attendances` row with `status = wfh`, `source = wfh` (frozen days and days already covered by approved leave are left untouched).
...
- **workflow_definitions**: id, module (leave/attendance_regularization/expense/loan/salary_advance/resignation/work_from_home/other), name, is_active
...
- **employee_tax_regimes**: id, employee_id -> employees, financial_year_id -> financial_years, selected_regime, selection_date, lock_date, changed_by. History row per change (no overwrite). `lock_date` reached (set via the admin "Fix / Lock" action) blocks further self-service changes — see §8
...
- **form16s**: id, employee_id -> employees, financial_year_id -> financial_years (unique together), regime, pdf_path, generated_at, generated_by -> users. One row per employee per FY (`updateOrCreate`d on regenerate)
...
Financial Year + regime slabs/config configured once (immutable once payroll runs reference it)
 -> Employee selects Old or New regime via TaxRegimeSelectionService::select() (employee_tax_regimes,
    append-only). Blocked for anyone without `tax.manage` once EITHER:
      - the current row's lock_date has arrived (HR used the admin "Fix / Lock" action), OR
      - TaxRegimeConfig.regime_change_allowed = false for that FY + regime (blanket freeze)
    `tax.manage` users (HR/Payroll) always bypass both — that's how they "fix" an employee's
    regime regardless of lock state. Unlock reverses a row-level lock.
 ...
 -> At financial year end (or on demand), HR/Payroll generates Form 16 (Form16Service::generate())
    from the same projection + verified declarations + quarter-wise TDS from payroll — one PDF
    per employee per FY, downloadable by HR/Payroll or the employee themselves
```
...
7. **Phase 7 – Form 16 & Regime Lock**: Employer TAN → Regime Lock/Unlock Enforcement → Form 16 PDF Generation → Admin Bulk Generation → ESS Download
